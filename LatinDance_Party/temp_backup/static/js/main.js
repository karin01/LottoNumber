...
function displayParty(party, containerId = 'parties-container', isPastParty = false) {
    ...
    partyCard.innerHTML = `
        ...
        <div class="party-info">
            <span style="font-size:16px;vertical-align:middle;margin-right:4px;">🏠</span><strong>장소:</strong> ${party.location}
        </div>
        <div class="party-info">
                ...
                <div class="modal-body">
                    <div id="modal-party-info">
                        <div class="party-detail-info">
                            ...
                            <div class="party-info-row">
                                <strong>🏠 장소:</strong> ${party.location || '장소 미정'}
                            </div>
                            <div class="party-info-row">
                                <div style="display: flex; align-items: center; flex: 1;">
                                    <strong>🗺️ 주소:</strong>
...
function closePartyModal() {
    ...
    if (modal) {
        if (modal.classList.contains('hidden')) {
            // hidden 클래스가 있는 경우 (기존 모달)
            modal.classList.add('hidden');
        } else {
            // hidden 클래스가 없는 경우 (동적 생성된 모달)
            modal.remove();
        }
    }
...
function playYouTubeVideo(video) {
    ...
    // 모달이 없으면 생성
    if (!modal) {
        console.log('영상 재생 모달이 없어서 생성합니다...');
        modal = document.createElement('div');
            ...
            <div class="modal-content youtube-modal-content">
                ...
                <div class="modal-body">
                    <div class="youtube-video-container">
                        <iframe id="youtube-video-frame" src="" frameborder="0" allowfullscreen></iframe>
                    </div>
                    <div class="youtube-video-info">
                        <h4 id="youtube-video-category"></h4>
        ...
        `;
        document.body.appendChild(modal);
        console.log('영상 재생 모달 생성 완료');
    }
    
    // 모달 요소들 찾기
    const iframe = document.getElementById('youtube-video-frame');
    const title = document.getElementById('youtube-video-title');
    const category = document.getElementById('youtube-video-category');
    const description = document.getElementById('youtube-video-description');
    const date = document.getElementById('youtube-video-date');
    const author = document.getElementById('youtube-video-author');
    
    console.log('모달 요소들:', { modal, iframe, title, category, description, date, author });
    
    if (modal && iframe) {
        // iframe src 설정
        const embedUrl = `https://www.youtube.com/embed/${video.videoId}?autoplay=1&rel=0`;
        iframe.src = embedUrl;
        console.log('iframe src 설정:', embedUrl);
        
        // 정보 설정
        if (title) title.textContent = video.title;
        if (category) category.textContent = `카테고리: ${video.category}`;
        if (description) description.textContent = video.description || '설명 없음';
        if (date) date.textContent = `등록일: ${new Date(video.createdAt).toLocaleDateString('ko-KR')}`;
        if (author) author.textContent = `등록자: ${video.author}`;
        
        // 모달 표시
        modal.classList.remove('hidden');
        console.log('영상 재생 모달 표시 완료');
    } else {
        console.error('모달 또는 iframe을 찾을 수 없습니다!');
    }
}
