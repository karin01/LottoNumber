...
- **당첨번호 조회**는 dhapi에 포함되어 있지 않습니다. 이 프로젝트는 우선 `common.do?method=getLottoNumber&drwNo=회차`를 호출하고, 일부 IP에서 차단되면 공식 추첨결과 페이지(`lt645/result`)와 동일 출처의 **`lt645/selectPstLt645Info.do?srchLtEpsd=회차`** JSON으로 폴백합니다.
